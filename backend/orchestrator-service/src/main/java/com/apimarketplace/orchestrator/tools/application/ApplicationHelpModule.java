...
public class ApplicationHelpModule implements ToolModule {
    ...
    private ToolExecutionResult executeHelp(Object topicsRaw) {
        ...
        if (all || topics.contains("troubleshooting")) result.put("troubleshooting", Map.of(
            ...
            "acquire_permission_denied_self_hosted_only", "If acquire returns PERMISSION_DENIED saying the app is Community Edition exclusive, the app uses features that only run on a self-hosted install (see its `ce_exclusive_features`: CLI_AGENT = a local CLI agent, VECTOR_SEARCH = embedding columns). This is TERMINAL on this deployment: retrying, changing parameters, or re-calling with a different trigger cannot succeed, and no action available to you can install it. Say so plainly and suggest an app without `ce_exclusive`, or building the equivalent yourself starting from workflow(action='init', name='<name>').",
    ...
    private Map<String, Object> buildResponseFieldDocs() {
        ...
        r.put("ce_exclusive", "boolean - present (and always true) ONLY when the app needs a self-hosted install AND this deployment cannot provide one. ABSENT means installable here, including on a self-hosted install browsing the same catalogue. When present, acquire is REFUSED with PERMISSION_DENIED: do not call acquire, tell the user the app is self-hosted-only, and offer another app instead.");
        r.put("ce_exclusive_features", "array of strings - why the app is self-hosted-only, e.g. ['CLI_AGENT'] (it uses a local CLI agent) or ['VECTOR_SEARCH'] (it uses embedding columns / similarity search). Present only alongside ce_exclusive. Use it to explain the limitation to the user in plain words.");
        return r;
    }
