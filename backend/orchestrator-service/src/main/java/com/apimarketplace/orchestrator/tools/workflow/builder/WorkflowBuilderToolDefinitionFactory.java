    ...
    public AgentToolDefinition buildToolDefinition() {
        List<ToolParameter> params = List.of(
            stringParam("action", "Action: init, load, save, finish, add_node, run_node, connect, disconnect, modify, remove, undo, describe, validate, search, execute, stop_run, restart_from_node, get, list, delete, runs, get_run, wait_run, get_node_output, get_plan, set_plan, pin, unpin, publish, unpublish, resolve_approval, continue_interface, mock_suggest, help. " +
                "'restart_from_node' re-runs ONE node of an existing run and continues from there, keeping everything the run already produced upstream (run_id + node). Prefer it over execute when a run got most of the way and one node produced the wrong result or failed: execute redoes the whole workflow and pays for it again. Read the response's 'outcome' before assuming the replay finished. " +
                "'run_node' runs ONE node immediately from a config, with no workflow and no run: use it to check a config works before building it in. Params: type + params (the node config, read ONLY from params - no top-level argument is used as config) + optional run_input and label. Nothing is saved; the response echoes the config back with credentials redacted so you can paste it into add_node. Types that pause on a signal (approval, interface, wait) or that only mean something inside a DAG (merge, aggregate, split, fork, loop) are refused with an explanation. Gated by the chat authorization card, like execute, and unavailable outside a conversation. " +
