...
One-command launcher for **Trinyx Community Edition**. It wraps the Docker
...
```bash
npx trinyx          # start (default): pulls images and boots the stack
npx trinyx down     # stop and remove the containers
npx trinyx logs     # follow the logs
npx trinyx status   # container status
npx trinyx update   # pull the latest images and restart
```
...
[the repository](https://github.com/TrinyxAI/Trinyx) and run
...
Data lives in `./trinyx` next to where you run the command (remove it to
reset). Optional configuration (LLM keys, SMTP, ports) is documented in the main
project README; copy `./trinyx/.env.example` to `.env` and re-run.

Licensed under AGPL-3.0. Part of https://github.com/TrinyxAI/Trinyx
