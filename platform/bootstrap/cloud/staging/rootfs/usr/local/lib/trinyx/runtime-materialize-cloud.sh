...
# Ces deux fichiers appartiennent exclusivement au bootstrap pre-Docker.
...
install -o root -g root -m 0600 \
    "$CONFIG/cloud-paid.override.yml" \
...
for name in required:
    if not re.fullmatch(r"[A-Z][A-Z0-9_]*", name):
        raise SystemExit(
            f"ERROR_INVALID_CLOUD_SSM_SCHEMA_NAME={name}"
        )
...
with open(static_path, "r", encoding="utf-8") as fh:
    for raw in fh:
        ...
        if not re.fullmatch(r"[A-Z][A-Z0-9_]*", name):
            raise SystemExit(
                f"ERROR_INVALID_CLOUD_STATIC_NAME={name}"
            )

        if name in static:
            raise SystemExit(
                f"ERROR_DUPLICATE_CLOUD_STATIC_NAME={name}"
            )
...
for parameter in data.get("Parameters", []):
    ...
    if not re.fullmatch(r"[A-Z][A-Z0-9_]*", name):
        raise SystemExit(
            f"ERROR_INVALID_CLOUD_SSM_NAME={name}"
        )

    if name in values:
        raise SystemExit(
            f"ERROR_DUPLICATE_CLOUD_SSM_NAME={name}"
        )
...
if missing:
    print(
        "ERROR_MISSING_CLOUD_SSM_NAMES=" + ",".join(missing),
        file=sys.stderr,
    )

if extra:
    print(
        "ERROR_EXTRA_CLOUD_SSM_NAMES=" + ",".join(extra),
        file=sys.stderr,
    )
...
if address.version != 4 or not internal_bind.startswith("10.30."):
    raise SystemExit("ERROR_INVALID_CLOUD_INTERNAL_BIND")
...
with open(output_path, "x", encoding="utf-8", newline="\n") as fh:
    ...
    for name in sorted(static):
        fh.write(
            f"export {name}={shlex.quote(static[name])}\n"
        )

    fh.write(
        "export CLOUD_INTERNAL_BIND="
        + shlex.quote(internal_bind)
        + "\n"
    )
...
# Le JSON contient les réponses SSM déchiffrées :
# il ne doit jamais entrer dans la génération publiée.
rm -f "$TMP/ssm.json"
...
# La génération entière est complète avant activation.
mv "$TMP" "$FINAL"
TMP=""

# Point d'activation atomique.
...
publish_link \
    cloud-images.env \
    cloud-current/cloud-images.env

publish_link \
    cloud-paid.override.yml \
    cloud-current/cloud-paid.override.yml

publish_link \
    cloud-auth-files.sh \
    cloud-current/cloud-auth-files.sh

publish_link \
    cloud.runtime.active.sh \
    cloud-current/cloud.runtime.active.sh
...
case "$REAL" in
    "$GENERATIONS"/gen-*) ;;
    *)
        echo ERROR_INVALID_CLOUD_GENERATION
        exit 1
        ;;
esac
...
# Le materializer n'est jamais propriétaire des deux fichiers pre-Docker.
cmp -s \
    /opt/trinyx/private-ca/paid-monolith-truststore.p12 \
    "$RUNDIR/auth-runtime/paid-monolith-truststore.p12"

cmp -s \
    /etc/trinyx/secrets/paid-monolith-truststore-password \
    "$RUNDIR/auth-runtime/paid-monolith-truststore-password"
