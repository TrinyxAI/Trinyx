...
BASE=$(trinyx_env_get PAID_BASE)
SSM_PATH=$(trinyx_env_get PAID_SSM_PATH)
PAID_IP=$(trinyx_env_get PAID_PRIVATE_IP)

EXPECTED_BASE="/etc/trinyx/${ENVIRONMENT}/paid"
EXPECTED_SSM_PATH="/trinyx/${ENVIRONMENT}/paid/"
test "$BASE" = "$EXPECTED_BASE" || { echo ERROR_PAID_BASE_ENVIRONMENT_MISMATCH >&2; exit 1; }
test "$SSM_PATH" = "$EXPECTED_SSM_PATH" || { echo ERROR_PAID_SSM_PATH_ENVIRONMENT_MISMATCH >&2; exit 1; }
...
if address.version != 4 or not any(address in network for network in private_ranges):
    raise SystemExit("ERROR_INVALID_PAID_PRIVATE_IP")
PY
...
install -o root -g root -m 0600 "$ACTIVE/images.env" "$TMP/paid-images.env"
install -o root -g root -m 0600 "$CONFIG/paid.override.yml" "$TMP/paid.override.yml"
install -o root -g root -m 0600 "$CONFIG/paid-bind.override.yml" "$TMP/paid-bind.override.yml"
install -o root -g root -m 0600 "$CONFIG/paid-runtime.override.yml" "$TMP/paid-runtime.override.yml"
...
if missing:
    print("ERROR_MISSING_SSM_NAMES=" + ",".join(missing), file=sys.stderr)
if extra:
    print("ERROR_EXTRA_SSM_NAMES=" + ",".join(extra), file=sys.stderr)
if missing or extra:
    raise SystemExit(1)
...
publish_link paid-images.env paid-current/paid-images.env
publish_link paid.override.yml paid-current/paid.override.yml
publish_link paid-bind.override.yml paid-current/paid-bind.override.yml
publish_link paid-runtime.override.yml paid-current/paid-runtime.override.yml
publish_link paid-secrets.env paid-current/paid-secrets.env
