...
Ready-made entries for self-hosting platforms. Each one points at the canonical Trinyx CE bundle
`cli/assets/docker-compose.yml` instead of restating it, so a change to the stack can never leave a
...
```
https://raw.githubusercontent.com/TrinyxAI/Trinyx/main/templates/portainer/trinyx-ce.json
```

Trinyx CE then appears under **App Templates**, with the ports, database password
...
point them at this repository's canonical `cli/assets/docker-compose.yml`.
...
2. **Only if the backend is not at `<the address you open the app with>:BACKEND_PORT`** -
   ...
   `https://trinyx.example.com`. Otherwise leave it empty: the app derives the
